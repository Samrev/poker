...
const getRankCount = (ranks) => ranks.reduce((count, rank) => {
    count[rank] = (count[rank] || 0) + 1;
    return count;
}, {});
const isSameSuit = (suits) => suits.every((suit) => suit === suits[0]);
const isSeq = (ranks) => ranks.every((rank, i, arr) => i === 0 || rank === arr[i - 1] + 1);
const isRoyalFlush = (suits, ranks) => isSameSuit(suits) &&
...
const isFullHouse = (rankCount) => Object.values(rankCount).includes(3) && Object.values(rankCount).includes(2);
...
const isTwoPair = (rankCount) => Object.values(rankCount).filter((count) => count === 2).length === 2;
...
const pokerWinner = (pokerCards, playersCards, playersStatus) => {
    let bestHandRank = 10;
    let bestRanks = [];
    let winners = [];
    for (const [player, playerCards] of Object.entries(playersCards)) {
        if (playersStatus[player] === vars_1.PlayerStatus.FOLDED)
            continue;
...
const rewardWinners = (pokerCards, playerCards, playersStatus, contributedPlayersBids, playersBalances, potBalance) => {
    const winningData = pokerWinner(pokerCards, playerCards, playersStatus);
    const allInWinners = [];
    const inGameWinners = [];
    for (const player of winningData.winners) {
        if (playersStatus[player] === vars_1.PlayerStatus.IN_GAME) {
    ...
    for (const [player, bid] of Object.entries(contributedPlayersBids)) {
        for (const winner of allInWinners) {
            const add = Math.floor(Math.min(contributedPlayersBids[winner] / winningData.winners.length, bid / winningData.winners.length));
...
exports.rewardWinners = rewardWinners;
